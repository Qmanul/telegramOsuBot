...
async def db_start():
    async with aiosqlite.connect('new.db') as db:
        await db.execute("CREATE TABLE IF NOT EXISTS profile"
                         "(telegram_user_id TEXT PRIMARY KEY NOT NULL, username TEXT, user_id TEXT, gamemode TINYINT"
                         ")")
        await db.commit()
...
async def create_new_user(user, osu_user):
    new_user = {
...
async def check_user_exists(user_id):
    async with aiosqlite.connect('new.db') as db:
        async with db.execute("SELECT * FROM profile WHERE telegram_user_id = '{user_id}'".format(user_id=user_id)) as cur:
